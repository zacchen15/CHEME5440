...
AMPstep = 0.01
AMPstop = 1.0
AMP_concentration = data[:,1]

# things that were estimated in part a
W1 = 0.05
W2 = 20.0

# things that need estimating
AMP_binding_constant_array = collect(0.0:0.1:0.1)
n_array = collect(0.0:0.1:0.1)
...
u_array = Array{Float64}(undef, length(AMP_binding_constant_array), length(n_array), length(AMP_concentration))
error_array = Array{Float64}(undef, length(AMP_binding_constant_array), length(n_array))
# iterate through the range of AMP binding
for j in 1:length(AMP_binding_constant_array)
        AMP_binding_constant = AMP_binding_constant_array[j]
        for k in 1:length(n_array)
                n = n_array[k]
                # do that calculations for the rate
                ...
                #u_array[j,k,:] = u
                #error_array[j,k] = overall_rate_array[j,k,:] - data[:,2]
        end
end
